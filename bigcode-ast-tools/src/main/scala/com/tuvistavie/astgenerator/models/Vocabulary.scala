...
object Vocabulary {
  ...
  def fromTSV(tsv: String): Vocabulary = {
    ...
    val vocabItems = linesIterator.foldLeft(Map.empty[Int, SubgraphVocabItem]) { case (items, row) =>
      val item = row.split("\t")
      val hasValue = item.isDefinedAt(4) && item(4).length > 0
      val token = Token(item(1), hasValue.option(item(4)))
      items + (item(0).toInt -> SubgraphVocabItem(Subgraph(token), item(3).toInt))
    }
    new Vocabulary(vocabItems, 1, strippedIdentifiers)
  }
}
